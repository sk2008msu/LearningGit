function [hit_rays, parent_hit_rays] = final_iterator(wall_edges,goal_box_edges,rays,radian_tick,min_acceptable_power)
...
for ray_count = 1:1:size(rays,2) %set to 0:1:size(rays,2) for production; for loop allows for breath iteration  
    ray_flag = 0;
    ...
    while(ray_flag == 0) % while loop allows for depth ray iteration
        ...
        if key_index > 0
            ...
            if new_ray_power > min_acceptable_power
                ...
                new_ray.final_point = [hits(key_index,1) hits(key_index,2)];
